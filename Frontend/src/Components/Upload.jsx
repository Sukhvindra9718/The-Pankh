...
function Upload({ setIsUpload, setUploadFormOpen, Uploadtitle, UploadType }) {
  ...
  const handleUploadVideo = async (e) => {
    ...
    if (title) {
      ...
      await fetch("http://165.227.97.26:3001/api/v1/video/upload", {
        mode: "no-cors",
        method: "POST",
        body: formData,
      })
  ...
  const handleUploadImage = async (e) => {
    ...
    if (title) {
      ...
      try {
        const { data } = await axios.post("http://165.227.97.26:3001/api/v1/image/upload",{title,description,image},config);
        console.log(data);
      } catch (error) {
        console.log(error);
      }
