...
function EventsOverview() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const { data } = await axios.post(
        "http://165.227.97.26:3001/api/v1/events/upload",
        { ...events, file },
        config
      );
  ...
  const handleUpdate = async () => {
    ...
    try {
      const { data } = await axios.put(
        `http://165.227.97.26:3001/api/v1/events/${updateId}`,
        Data,
        config
      );
  ...
  const getAllEvents = async () => {
    try {
      const res = await axios.get("http://165.227.97.26:3001/api/v1/events");
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm) {
      ...
      const { data } = await axios.delete(`http://165.227.97.26:3001/api/v1/events/${id}`, config);
