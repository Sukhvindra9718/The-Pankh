...
function NewsOverview() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const { data } = await axios.post(
        "http://165.227.97.26:3001/api/v1/news/upload",
        { ...news, file },
        config
      );
  ...
  const getAllNews = async () => {
    try {
      const res = await axios.get("http://165.227.97.26:3001/api/v1/news");
      if (res.data.success) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const { data } = await axios.put(
        `http://165.227.97.26:3001/api/v1/news/${updateId}`,
        Data,
        config
      );
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm) {
      ...
      const { data } = await axios.delete(`http://165.227.97.26:3001/api/v1/news/${id}`, config);
