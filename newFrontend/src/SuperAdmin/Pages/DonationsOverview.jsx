...
function DonationsOverview() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const { data } = await axios.put(
        `http://localhost:3001/api/v1/Donation/${updateId}`,
        Data,
        config
      );
  ...
  const getAllDonations = async () => {
    try {
      const res = await axios.get("http://localhost:3001/api/v1/Donations");
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm) {
      ...
      const { data } = await axios.delete(`http://localhost:3001/api/v1/Donation/${id}`, config);
