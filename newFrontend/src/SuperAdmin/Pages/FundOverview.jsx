...
function FundOverview() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const { data } = await axios.post(
        "http://165.227.97.26/api/v1/fundDetails/upload",
        { ...fund, file },
        config
      );
  ...
  const handleUpdate = async () => {
    ...
    try {
      const { data } = await axios.put(
        `http://165.227.97.26/api/v1/fundDetails/${updateId}`,
        Data,
        config
      );
  ...
  const getAllfund = async () => {
    try {
      const res = await axios.get("http://165.227.97.26/api/v1/fundDetails");
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm) {
      ...
      const { data } = await axios.delete(`http://165.227.97.26/api/v1/fundDetails/${id}`, config);
